    ...
    if msgs > max_msg then
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  7 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر به دلیل اسپم بیش از حد بن گولبال شد\nنام : "..name.."\nایدی : "..msg.from.id)
		  send_large_msg("channel#id"..msg.to.id, "کاربر به دلیل اسپم بیش از حد بن گولبال شد\nنام : "..name.."\nایدی : "..msg.from.id)
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "گاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." به دلیل اسپم بیش از حد بن گولبال شد ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] "
