...
return msg
			...
			if is_link_msg and lock_link == "yes" and not is_bot then
				...
				if strict == "yes" or to_chat then
					if link_flood == 'ban' then
					  send_msg("chat#id"..msg.to.id, 'کاربر ('..USERNAME..') به دلیل تبلیغ کردن بن شد',ok_cb,false)
					  ban_user(msg.from.id, msg.to.id)
					elseif link_flood == 'kick' then
					  send_msg("chat#id"..msg.to.id, 'کاربر ('..USERNAME..') به دلیل تبلیغ کردن اخراج شد',ok_cb,false)
					  kick_user(msg.from.id, msg.to.id)
				end
			end
		end
		...
		if msg.media then -- msg.media checks
			if msg.media.title then
				...
				local is_eng_title = msg.media.title:match("[Aa][Bb][Cc][Dd][Ee][Ff][Gg][Hh][Ii][Jj][Kk][Ll][Mm][Nn][Oo][Pp][Qq][Rr][Ss][Tt][Uu][Vv][Ww][Xx][Yy][Zz]")
			...
			if is_emoji_title and lock_emoji == "yes" then
				...
				if is_link_title and lock_link == "yes" then
					...
					if strict == "yes" or to_chat then
							if link_flood == 'ban' then
							  send_msg(get_receiver(msg), 'کاربر ('..USERNAME..') به دلیل تبلیغ کردن بن شد',ok_cb,false)
							  ban_user(msg.from.id, msg.to.id)
							elseif link_flood == 'kick' then
							  send_msg(get_receiver(msg), 'کاربر ('..USERNAME..') به دلیل تبلیغ کردن اخراج شد',ok_cb,false)
							  kick_user(msg.from.id, msg.to.id)
							end
					end
				end
			...
			local is_eng_desc = msg.media.description:match("[Aa][Bb][Cc][Dd][Ee][Ff][Gg][Hh][Ii][Jj][Kk][Ll][Mm][Nn][Oo][Pp][Qq][Rr][Ss][Tt][Uu][Vv][Ww][Xx][Yy][Zz]")
			...
			if is_tag_caption and lock_tag == "yes" then
				...
				local is_english_caption = msg.media.caption:match("[Aa][Bb][Cc][Dd][Ee][Ff][Gg][Hh][Ii][Jj][Kk][Ll][Mm][Nn][Oo][Pp][Qq][Rr][Ss][Tt][Uu][Vv][Ww][Xx][Yy][Zz]")
			if is_english_caption and lock_english == "yes" then
				delete_msg(msg.id, ok_cb, false)
				if strict == "yes" or to_chat then
					kick_user(msg.from.id, msg.to.id)
				end
					...
					if to_chat then
							  ...
							  if group_sticker_lock == 'yes' then
								  if is_sticker_offender then
									...
									send_msg("chat#id"..msg.to.id, 'به دلیل ارسال استیکر از گروه اخراج شد', ok_cb, false)
								  else
									redis:set(sticker_hash, true)
									send_msg("chat#id"..msg.to.id, 'شما به دلیل ارسال استیکر اخطار دریافت کردید \nدر صورت ارسال دوباره استیکر از گروه اخراج میشوید!', ok_cb, false)
								  end
							  end
					end
				end
