...
function run(msg, matches)
	if matches[1]:lower() == 'تنظیم اعتبار' then
	...
	if matches[1]:lower() == 'نامحدود' then
		if not is_sudo(msg) then return end
		redis:hdel('expiretime',get_receiver(msg))
		return "تاریخ انقضای گروه:\nبه نامحدود تنظیم شد."
	end
	if matches[1]:lower() == 'اعتبار گروه' then
		local expiretime = redis:hget ('expiretime', get_receiver(msg))
		if not expiretime then return 'تاریخ ست نشده است' else
			local now = tonumber(os.time())
			return (math.floor((tonumber(expiretime) - tonumber(now)) / 86400) + 1).. "روز تا پایان تاریخ انقضای گروه"
		end
	end
