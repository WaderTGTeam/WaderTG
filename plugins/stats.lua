...
local function bot_stats()
  ...
  local text = '<code>کاربران</code>: <b>'..r..'</b>'

  hash = 'chat:*:users'
  r = redis:eval(redis_scan, 1, hash)
  text = text..'\n<code>گروه-ها</code>: <b>'..r..'</b>'
  return text
end
local function run(msg, matches)
  ...
  if matches[1]:lower() == "امار گروه" then
    if not is_momod(msg) then
      return "فقط میران می توانند شما دسترسی ندارید!"
    end
...
return {
  patterns = {
    ...
    "^(امار گروه)$",
    "^(امار) (گروه) (%d+)",
    "^(امار) ([Ww][Aa][Dd][Ee][Rr][Tt][Gg])",
	"^[#!/]([Ww][Aa][Dd][Ee][Rr][Tt][Gg])",
	"^([Ww][Aa][Dd][Ee][Rr][Tt][Gg])",
    }, 
  run = run
}
